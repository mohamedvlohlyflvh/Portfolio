...
export default function HoverImageReveal({
  ...
  /* Touch devices (pointer: coarse) get the plain row list — no chase.
     The reveal image is pointless without a hover cursor, and following
     taps makes the rows feel jumpy. */
  useEffect(() => {
  ...
  /* Warm the browser cache on first pointer entry so the reveal image paints
     instantly — zero white flash on the first hover. */
  const preloadImages = () => {
      ...
      {!coarse && (
        <motion.div
          style={{
            ...
            pointerEvents: "none",
            zIndex: 2,
            /* Layer promoted only while the chase is live; cleaned up after. */
            willChange: anyActive ? "transform, opacity" : "auto",
          }}
